...
void Car::countLaps()
{
    if(x<checkP1x && y<checkP1y && checkPoint1==false)
    {
        ...
        std::chrono::steady_clock::time_point time_init2 = std::chrono::steady_clock::now();
        if(lapCount<=0) lapTime = 0;
        else lapTime = std::chrono::duration_cast<std::chrono::microseconds>(time_init2 - time_init).count()/1000000.;
...
int main(void)
{
    ...
    cars[0]->Initialize(1,1,35);
    cars[1]->Initialize(2,0,35);
    cars[2]->Initialize(3,1,42);
    cars[3]->Initialize(4,0,42);
    cars[4]->Initialize(5,1,49);
    cars[5]->Initialize(6,0,49);
    ...
    for(;;)
    {
        ...
        for(int i=0; i<cars.size(); ++i)
        {
            if(tPassed>5)
            {
                if(i==1 ||i==3 || i==5) cars[i]->Move();
                
            }
            if(tPassed>9.5)
            {
                if(i==0 ||i==2 || i==4)cars[i]->Move();
            }
        }
    ...
    for(int i=0;i<6;++i) delete cars[i];
