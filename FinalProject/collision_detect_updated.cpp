...
/*****************************************************************************************************************/
// do unit test
const double colSize=50.0;
int main(void)
{
    player p = player((int)windowWidth/2, (int)windowHeight/2, colSize, colSize, 0.0, 200.0);
    std::vector<obstacle> obstacles;
    std::vector<enemy> enemies;
    std::vector<item> items;
    items.push_back(item(windowWidth-200, windowHeight/2, colSize, colSize, 0.0, 0.0, 1));
	srand((int)time(nullptr));

	for(int i=0; i<10; ++i)
	{
		double obx=(double)((rand()%(int)windowWidth)-150);
		double oby=(double)((rand()%(int)windowHeight)-400);
        obstacles.push_back(obstacle(obx, oby, colSize, colSize, 100.0, -100.0));

		double enx=(double)((rand()%(int)windowWidth)-150);
		double eny=(double)((rand()%(int)windowHeight)-400);
        enemies.push_back(enemy(enx, eny, colSize, colSize, -100.0, 100.0));
	}
    ...
    while(FSKEY_ESC!=FsInkey())
    {
        ...
        checkCollision(p, obstacles, enemies, items, itemHit);
        printf("%d\n",itemHit);
